...
import { useState, useEffect } from "react";
 const handleButtonClicker = () => {
    ...
    window.location.href ='/dashboard/AjouterEtudiant'; // Replace '/your-other-page' with the actual path you want to navigate to
  };
...
const [data, setData] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios.get(
        "http://localhost:8080/api/admin/etud"
      );
      setData(result.data);
 ...
 const handleDelete = async (id) => {
  try {
    ...
    await axios.delete(`http://localhost:8080/api/admin/etudiants/${id}`);
  ...
  Ajouter Etudiant
        ...
        <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
          <table className="w-full min-w-[640px] table-auto">
            <thead>
              <tr>
                {["Nom complet", "Promotion", "sexe", "action"].map((el) => (
                  <th
                    key={el}
            ...
            <tbody>
              {data && data.length > 0 && data.map(
                ({ nometudiant, prenometudiant, emailetudiant, annee_promo, sexetudiant }, key) => {
                  ...
                  return (
                    <tr key={nometudiant}>
                      ...
                      <Typography as="div" className="flex items-center">
                        <Link to="/dashboard/ModifyEtudiant">
